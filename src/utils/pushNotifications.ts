 ...
 * Implementează livrare Web Push securizată prin backend API (/api/send-push și /api/send-push.php).
 */
export async function broadcastPushNotification({
  ...
  // 2. Apel securizat către serverul backend de Web Push (/api/send-push și /api/send-push.php)
    ...
    try {
      const res = await fetch(ep, {
        ...
        headers: { 
          ...
          'X-Camena-Push-Auth': 'camena_internal_secure_dispatch_2026'
        },
        body: JSON.stringify({
          title,
          body,
