...
const ViewPayments = ({ members, onUpdateMember, isAdmin }: { members: any[], onUpdateMember: (m: any) => void, isAdmin: boolean }) => {
  ...
  useBodyScrollLock(!!receiptToRevert);
        ...
        {receiptToRevert && (
          <div className="fixed inset-0 z-[150] flex items-center justify-center p-3 sm:p-4 font-anthropic">
            ...
            <motion.div 
              ...
              className="relative w-full max-w-md max-h-[calc(100dvh-2rem)] sm:max-h-[88vh] overflow-y-auto overscroll-contain bg-white dark:bg-[#161B22] rounded-[2px] p-5 sm:p-8 shadow-2xl border border-slate-300 dark:border-slate-700 font-anthropic z-10 touch-pan-y"
        ...
        {showWarning && (
          <div className="fixed inset-0 z-[150] flex items-center justify-center p-3 sm:p-4 font-anthropic">
            ...
            <motion.div 
              ...
              className="relative w-full max-w-md max-h-[calc(100dvh-2rem)] sm:max-h-[88vh] overflow-y-auto overscroll-contain bg-white dark:bg-[#161B22] rounded-[2px] shadow-2xl p-5 sm:p-8 z-10 border border-slate-300 dark:border-slate-700 font-anthropic touch-pan-y"
              style={{ WebkitOverflowScrolling: 'touch' }}
            >
...
const ViewProfile = ({ currentUserObj, onUpdateMember }: ViewProfileProps) => {
  ...
  useBodyScrollLock(!!tempImageSrc);
  ...
  const handleSave = async () => {
    setIsSaving(true);
    try {
      // Dacă s-a introdus o parolă nouă, o actualizăm prin funcția securizată RPC change_member_password
      if (newPassword && newPassword.trim().length > 0) {
        if (!oldPassword || oldPassword.trim().length === 0) {
          toast.error("Te rugăm să introduci parola actuală pentru a putea seta una nouă.");
          setIsSaving(false);
          return;
        }
        ...
        const { data: passRes, error: passErr } = await supabase.rpc('change_member_password', {
          p_member_id: effectiveUser.id,
          p_old_password: oldPassword.trim(),
          p_new_password: newPassword.trim()
        });

        if (passErr || (passRes && !passRes.success)) {
          toast.error(passRes?.error || passErr?.message || "Parola actuală este incorectă.");
          setIsSaving(false);
          return;
        } else {
    ...
    <div className="w-full space-y-8 max-w-7xl mx-auto">
              ...
              <div className="space-y-2">
                ...
                <div className="space-y-2">
                  <input 
                    ...
                    placeholder="Parola actuală (obligatorie pentru schimbare)" 
                    className="w-full px-4 py-2.5 bg-slate-50 dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded-[2px] text-slate-900 dark:text-white text-sm font-medium focus:outline-none focus:border-sky-500 font-anthropic"
                  />
                  <div className="relative">
                    <input 
                      ...
                      placeholder="Noua parolă (minim 6 caractere)" 
      ...
      {tempImageSrc && (
        <div className="fixed inset-0 z-[150] flex items-center justify-center p-3 sm:p-4 font-anthropic">
          ...
          />
          <div 
            className="relative bg-white dark:bg-slate-900 rounded-[2px] p-5 sm:p-6 max-w-md w-full max-h-[calc(100dvh-2rem)] sm:max-h-[90vh] overflow-y-auto overscroll-contain border border-slate-300 dark:border-slate-800 shadow-2xl flex flex-col items-center font-anthropic z-10 touch-pan-y"
            style={{ WebkitOverflowScrolling: 'touch' }}
          >
