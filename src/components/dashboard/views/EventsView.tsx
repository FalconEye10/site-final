...
export function EventsView({ isAdmin, members = [], currentUserId, currentUserObj, onUpdateMember }: EventsViewProps) {
  ...
  return (
    <div className="space-y-6 relative font-anthropic">
              ...
              <div className="flex flex-col sm:flex-row sm:items-center gap-5">
                ...
                <div className="space-y-1.5 flex-1 font-anthropic">
                  ...
                  <div className="text-xs sm:text-sm font-medium text-slate-600 dark:text-slate-300 flex flex-wrap items-center gap-x-3.5 gap-y-1">
                    <span className="flex items-center gap-1.5"><Calendar size={14} className="text-slate-400 shrink-0" /> {formatRomaniaDate(nextEvent.date, { weekday: 'long', day: 'numeric', month: 'long' })}</span>
                    <span className="flex items-center gap-1.5"><Clock size={14} className="text-slate-400 shrink-0" /> {nextEvent.time}</span>
                    {nextEvent.location && <span className="flex items-center gap-1.5"><MapPin size={14} className="text-slate-400 shrink-0" /> {nextEvent.location}</span>}
                  </div>
                </div>
              </div>
            ) : (
        ...
        {isAdmin && (
          <div className="flex items-center shrink-0">
            <button 
              ...
              className="btn-civic-primary px-3.5 py-2 text-xs font-bold font-title uppercase tracking-wider flex items-center gap-1.5 w-full lg:w-auto justify-center shadow-xs cursor-pointer"
            >
              <Plus size={15} /> Creează Eveniment Nou
            </button>
          </div>
        )}
      </div>
      ...
      {/* View Tabs */}
      <div className="flex gap-2 border-b border-slate-200 dark:border-slate-800 pb-2 font-title">
        <button 
          type="button"
          onClick={() => setActiveViewTab('upcoming')}
          className={`px-3 py-1.5 rounded-[2px] text-xs font-bold uppercase tracking-wider transition-all cursor-pointer shadow-xs ${
          ...
          {Object.entries(groupedEvents).map(([monthYear, monthEvents]) => (
            <div key={monthYear} className="space-y-4">
              <div className="flex items-center gap-3 font-title">
                <span className="text-xs sm:text-sm font-bold uppercase tracking-widest text-slate-600 dark:text-slate-400 bg-slate-100 dark:bg-slate-800 px-3.5 py-1.5 rounded-[2px] border border-slate-200/80 dark:border-slate-700 flex items-center gap-1.5">
            ...
            >
              ...
              <form className="space-y-4 overflow-y-auto overscroll-contain flex-1 min-h-0 pr-1 -mr-1 scrollbar-thin touch-pan-y font-anthropic" style={{ WebkitOverflowScrolling: 'touch' }} onSubmit={handleSubmit}>
                ...
                <div className="pt-3 pb-1 sticky bottom-0 bg-white dark:bg-slate-900 border-t border-slate-100 dark:border-slate-800 z-10 font-title shrink-0">
                  <button type="submit" className="w-full py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-[2px] font-bold uppercase tracking-wider text-xs shadow-xs cursor-pointer">
